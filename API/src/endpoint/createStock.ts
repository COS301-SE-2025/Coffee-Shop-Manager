...
export async function createStockHandler(
  ...
  try {
    ...
    if (!Array.isArray(entries) || entries.length === 0) {
      res.status(400).json({
        error: "Request body must be a non-empty array of stock items",
      });
      return;
    }
    ...
    for (const entry of entries) {
      const { item, quantity, unit_type, max_capacity, reserved_quantity } =
        entry;
      ...
      if (reserved_quantity !== undefined)
        newStock.reserved_quantity = reserved_quantity;
