...
const highlights = [
  ...
  {
    id: "R1007",
    note: "Repeat customer (5th visit)",
    amount: "R320",
    customer: "Musa K.",
    time: "09:15",
    status: "completed",
...
export default function QuickReport() {
  ...
  const getChartData = () => {
    switch (selectedMetric) {
      ...
      default:
        return salesData.map((d) => ({ ...d, value: d.sales }));
    }
  };
        ...
        {summaryCards.map(({ title, value, icon, trend, trendColor }) => (
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <div
                className={`text-sm font-semibold ${trendColor} bg-gray-100 px-2 py-1 rounded-full`}
              >
                {trend}
              </div>
            </div>
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl border border-white/50">
          ...
          <div className="p-6">
            <div className="space-y-4">
              {highlights.map(
                ({ id, note, customer, amount, time, status }) => (
                  <div
                    key={id}
