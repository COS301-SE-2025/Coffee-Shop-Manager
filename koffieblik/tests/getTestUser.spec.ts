...
test("fetches and displays orders from /get_orders on userdashboard", async ({
  page,
}) => {
  await login(page);
...
test("fetches and displays points on userdashboard", async ({ page }) => {
  ...
  const seePointsDivMain = page
    .locator("h2", { hasText: /Your Loyalty Points/i })
    .first();
  await expect(seePointsDivMain).toBeVisible({ timeout: 10000 });
});

test("checks that the navbar shows only Dashboard, Order Here, Help (plus Logout & username)", async ({
  page,
}) => {
  await login(page);
  await page.goto("http://localhost:3000/userdashboard");
